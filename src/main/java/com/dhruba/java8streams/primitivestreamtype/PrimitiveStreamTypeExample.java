package com.dhruba.java8streams.primitivestreamtype;
...
public class PrimitiveStreamTypeExample {
	...
	public static void main(String[] args) {
				...
				try {
					Set<String> shakespeareWords = 
						Files.lines(Paths.get("src/main/resources/words.shakespeare.txt"))
						.map(word -> word.toLowerCase())
						.collect(Collectors.toSet());
					
					Set<String> scrabbleWords = 
							Files.lines(Paths.get("src/main/resources/ospd.txt"))
							.map(word -> word.toLowerCase())
							.collect(Collectors.toSet());
